...
const GoalNode: React.FC<GoalNodeProps> = ({ data }) => {
  ...
  const toggleTaskSelection = (taskId: number) => {
    const isAssigned = data.tasks?.some((task) => task.id === taskId);
    if (!isAssigned) {
  ...
  const isTaskAssigned = (taskId: number) => {
    return data.tasks?.some((task) => task.id === taskId) || false;
  };

  return (
    <div className="relative">
      <div className="p-4 rounded-lg shadow-md w-72 bg-white border border-gray-200 transition-all hover:shadow-lg">
...
export default React.memo(GoalNode);
